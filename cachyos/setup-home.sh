...
COMPONENTS=(
    ...
    "scripts|~/.local/bin helpers: select-monitors.sh, hdr-toggle, ros2-humble, vnc-server, remote, lerobot-verify, fastfetch-logo"
...
do_scripts() {
    dry "$BIN/{select-monitors.sh,hdr-toggle,ros2-humble,vnc-server,remote,lerobot-verify,fastfetch-logo}" && return
    mkdir -p "$BIN"
...
EOF
    ...
    chmod +x "$BIN/select-monitors.sh" "$BIN/hdr-toggle" "$BIN/ros2-humble" "$BIN/vnc-server" "$BIN/remote" "$BIN/lerobot-verify" "$BIN/fastfetch-logo"
}
